...
interface VideoInspectionCatalogProps {
  onSubcategoryChange?: (subcategoryId: string | null) => void;
}

export function VideoInspectionCatalog({ onSubcategoryChange }: VideoInspectionCatalogProps) {
  const [selectedSubcategory, setSelectedSubcategory] = useState<VideoInspectionSubcategory | null>(null);
  const [selectedProduct, setSelectedProduct] = useState<VideoInspectionProduct | null>(null);
  ...
  const handleProductClick = (product: VideoInspectionProduct) => {
    ...
    <div className="space-y-4">
      <AnimatePresence mode="wait">
        {!selectedSubcategory ? (
          ...
          >
            {videoInspectionData.subcategories.map((subcategory) => (
              <button
                key={subcategory.id}
