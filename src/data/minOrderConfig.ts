...
export function getMinOrderConfig(productName: string, vendorCode?: string): MinOrderConfig | null {
  return MIN_ORDER_PRODUCTS.find(config => {
    if (config.vendorCode && vendorCode && config.vendorCode === vendorCode) {
      return true;
    }
    if (config.productNameContains && productName.includes(config.productNameContains)) {
      return true;
    }
    return false;
  }) || null;
